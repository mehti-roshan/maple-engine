...
struct VulkanMemoryManager {
 ...
 public:
  // Default constructor
  VulkanMemoryManager() : allocator(nullptr) {}

  // Constructor: creates the VMA allocator
  VulkanMemoryManager(vk::raii::Instance& instance, vk::raii::PhysicalDevice& physicalDevice, vk::raii::Device& device) {
  ...
  // Optional: helper for creating VulkanImages (if you define a VulkanImage wrapper)
  template <typename TImageWrapper>
  TImageWrapper createImage(VkExtent3D extent, VkFormat format, VkImageUsageFlags usage, VmaMemoryUsage memoryUsage) {
    return TImageWrapper(allocator, extent, format, usage, memoryUsage);
  }
};

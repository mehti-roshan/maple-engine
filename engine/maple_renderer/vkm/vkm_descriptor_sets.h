...
struct DescriptorSets {
  ...
  struct CreateInfo {
    ...
    uint32_t count;
    std::span<const Layout> description;
  };
 ...
 private:
  ...
  std::vector<vk::DescriptorSetLayoutBinding> layoutToVk(std::span<const Layout> description) {
    ...
    for (const auto& v : description) {
      vkFormat.push_back({
        ...
        .stageFlags = maple::ToVulkan(v.usedStages),
      });
    }

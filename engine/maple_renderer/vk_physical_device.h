...
struct VulkanPhysicalDevice {
  ...
  std::optional<vk::Format> FindFirstSupportedFormat(std::span<vk::Format> formats, vk::ImageTiling tiling, vk::FormatFeatureFlags features) const {
    for (const auto& format : formats) {
      auto formatProps = device.getFormatProperties(format);
